...
// Validation sources for this package:
//
//   - Chandler and Angstrom: the formulas are identical to the firebehavioR R
//     package (fireIndex, Sharples et al. 2009). A value here is reproducible by
//     that prior implementation. The R expression is given at each test.
...
//     values differ.
//   - Hot-Dry-Windy: Srock et al. 2018, vapour pressure deficit in hectopascals.
//     firebehavioR uses kilopascals and reports a value 10 times smaller.
//   - KBDI: the corrected Keetch and Byram (1968) analytic equation (constant
//     8.30). firebehavioR uses a metric lookup-table variant, so it is not a
//     numeric oracle for this analytic form.
//   - Nesterov: the classical sum, temperature times dew point depression, reset
//     above 3 mm of precipitation.
